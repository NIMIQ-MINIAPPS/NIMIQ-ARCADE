-- NIM-ARCADE — admin metrics dashboard (run after schema.sql, 002, 003, 005)
...
-- functions for the in-app admin metrics page (src/pages/AdminMetricsPage.tsx,
-- reachable at yourapp.com/#admin). Every function is `security definer`
...
--
-- HOW TO MAKE YOURSELF ADMIN (run once, after this file, and after you've
-- opened the app at least once so your player row exists):
--   insert into public.admin_players (player_id)
--   select id from public.players where nimiq_address = 'NQ... your address ...';
...
-- (which run as the table owner, bypassing RLS) can see it.
